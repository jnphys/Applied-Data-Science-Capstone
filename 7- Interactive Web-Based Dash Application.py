...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All Sites':
        success_df = data[data['class']==1]
        total_success_df = success_df.groupby(['Launch Site'])['class'].count().reset_index()
        fig = px.pie(total_success_df, values='class', names='Launch Site', 
            title='Total Success Launches for all Sites')
        return fig
    else:
        # return the outcomes piechart for a selected site
        site_df = data[data['Launch Site']==entered_site]
        site_success_df = site_df.groupby(['class']).count().reset_index()
        fig = px.pie(site_success_df, values='Launch Site', names='class', 
